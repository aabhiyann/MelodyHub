...
### Added
- **Top10 items 5–9 (social, chat, nav, animations, edge cases):** Manual QA guide [Docs/MANUAL_QA_LIVE_SITE.md](Docs/MANUAL_QA_LIVE_SITE.md) for live site (social/friends two-account, chat UX, navigation/IA, animations, edge cases). Playwright E2E can run against live URL via `BASE_URL=https://melodyhubmusic.vercel.app` and `npm run test:e2e:live` (after `npx playwright install`). Chat: 2000-char message limit with toast, `data-testid` for chat input and messages. E2E: guest visiting `/library` redirects to home.
- **QA checklist and sign-off process (Top10 #10):** Expanded `Docs/QA_CHECKLIST.md` to the 16-section structure from BETA_TEST_REPORT with desktop (1440×900) and mobile (390×844) viewports; added sign-off table. Added QA & Release section to CONTRIBUTING.md.
- **Final visual polish and documentation (sprint summary):** Page transition animations capped at 200ms (fade/slide) in `animation-variants`. Microinteractions: like button heart pulse (200ms), follow button tap animation on ProfileHeader, song like ripple effect via `createRipple` on shared LikeButton. Friendly empty states using shared `EmptyState` with messages and icons on PlaylistPage (empty playlist), QueueView (empty queue), SearchPage (no results), CreateEditPlaylistModal and DataTable (clear copy). User-friendly error messages for follow failure, featured section load, share/copy, and offline toast. App favicon (SVG), document title "MelodyHub – Music Streaming", theme-color `#22C55E`, and apple-touch-icon in `index.html`; logo already in TopBar/Sidebar (`/mascot/melody-icon.png`). README: environment variables table (frontend and backend), screenshots section with links to key pages on the deployed site.
...
### Fixed
- **Chat UX (Top10 #6):** Message input enforces 2000-character limit with user toast; send disabled when empty; `data-testid="chat-message-input"` and `data-testid="chat-messages"` for E2E.
...
- **Full regression QA (2026-02-27):** Re-ran full checklist on deployed site; SpotifyCard play button made always visible (opacity-90) so it is clickable without hover for keyboard/touch/automation. Updated `QA_REPORT.md` with results; all items PASS or N/A.
- **Mobile final pass (2026-02-27):** Back button on 404 (Topbar added to NotFoundPage); bottom padding on Community, Settings, Analytics, Followers, Following, Gamification so content clears bottom nav; 44px tap targets on SearchPage song rows, AddToPlaylistDialog rows, QueuePanel close, Community/Followers/Following/Analytics/Gamification buttons and tabs; momentum scroll (touch-scroll + overflow-x-hidden) on ScrollArea viewport, AddToPlaylistDialog, QueuePanel, LyricsPanel. See `MOBILE_FIXES.md` (Mobile Final Pass — 2026-02-27).
...
[Unreleased]: https://github.com/aabhiyann/MelodyHub/compare/v1.0.0...HEAD
