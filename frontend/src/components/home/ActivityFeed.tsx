...
export const ActivityFeed = () => {
    const { data: activities } = useQuery({
        ...
        queryFn: async () => {
            ...
            return response.data; // Assuming response is { activities: [] } or just []? 
            // Controller keys: handleSuccess(res, feed). Default base controller returns json(data) directly if array?
            // BaseController.handleSuccess(res, data) -> res.json(data).
            // So if feed is [], response.data is [].
            // If feed is { activities: [] }, then response.data.activities.
            // My activityService.getFeed returns array.
            // So response.data is Activity[].
