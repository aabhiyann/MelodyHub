	...
	return (
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			<DialogTrigger asChild>
				<Button
					variant='outline'
					className='w-full justify-start gap-2 text-zinc-400 hover:text-white border-zinc-800 hover:bg-zinc-800/50 hover:border-brand-primary/50 transition-all duration-300 group'
				>
					<Sparkles className='size-4 group-hover:text-brand-primary transition-colors' />
					AI Playlist
				</Button>
			</DialogTrigger>
			<DialogContent className='bg-zinc-950 border-zinc-800 max-w-md shadow-2xl shadow-brand-primary/10'>
				<DialogHeader>
					...
					<div className='space-y-2'>
						...
						<div className='flex gap-2'>
							<Input
								...
								className='bg-zinc-900 border-zinc-800 text-white placeholder:text-zinc-600 focus-visible:ring-brand-primary/50 h-11'
								onKeyDown={(e) => e.key === "Enter" && handleGenerate()}
							/>
							<Button
								onClick={handleGenerate}
								disabled={isLoading || !prompt.trim()}
								className='bg-brand-primary hover:bg-brand-primary/90 text-white h-11 px-6 font-medium shadow-glow-primary'
							>
								{isLoading ? (
					...
					{generatedPlaylist.length > 0 && (
						<div className='space-y-3 animate-in fade-in slide-in-from-top-2 duration-300'>
							<div className='flex items-center justify-between border-b border-zinc-800 pb-2'>
								...
								<Button size='sm' variant="ghost" onClick={handlePlay} className='gap-2 text-white hover:text-brand-primary hover:bg-zinc-800/50'>
