...
const PlaylistPage = () => {
    ...
    const handlePlayPlaylist = () => {
        ...
        const isCurrentPlaylistPlaying = populatedSongs.some((song) => song._id === currentSong?._id);
        if (isCurrentPlaylistPlaying) togglePlay();
        else {
            // start playing the playlist from the beginning
            playAlbum(populatedSongs, 0);
        }
    };
    ...
    const handlePlaySong = (index: number) => {
        ...
        playAlbum(populatedSongs, index);
    };

    const handleShuffle = () => {
        if (!populatedSongs.length) return;
        const shuffled = [...populatedSongs].sort(() => Math.random() - 0.5);
    ...
    // Safely filter to only fully-populated song objects (protect against unpopulated ObjectId refs)
    const populatedSongs = (currentPlaylist?.songs ?? []).filter(
        (s): s is NonNullable<typeof s> => typeof s === 'object' && s !== null && '_id' in s
    );
    const totalDurationSeconds = populatedSongs.reduce((acc, s) => acc + ((s as { duration?: number }).duration ?? 0), 0);
    ...
    const handleMoveUp = (index: number) => {
        if (!populatedSongs.length || !id || index <= 0) return;
        const songs = [...populatedSongs];
        [songs[index - 1], songs[index]] = [songs[index], songs[index - 1]];
        reorderSongs(id, songs.map((s) => s._id));
    };

    const handleMoveDown = (index: number) => {
        if (!populatedSongs.length || !id || index >= populatedSongs.length - 1) return;
        const songs = [...populatedSongs];
                            ...
                            <div className='flex flex-col md:flex-row p-6 gap-6 pb-8 items-center md:items-end'>
                                ...
                                <div className='flex flex-col justify-end text-center md:text-left flex-1 min-w-0'>
                                    ...
                                    <div className='flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-[#9CA3AF] justify-center md:justify-start'>
                                        ...
                                        <span className="flex items-center"><span className="w-1 h-1 rounded-full bg-[#6B7280] mx-2" /> {populatedSongs.length} songs</span>
                                        ...
                                        {populatedSongs.map((song, index) => {
                                            ...
                                            return (
                                                <PlaylistSongRow
                                                    ...
                                                    canMoveUp={index > 0}
                                                    canMoveDown={index < (populatedSongs?.length ?? 0) - 1}
                                                />
                                            );
                                        })}
                                    </div>
